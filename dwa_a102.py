...
from check_ranges import *
from climate import climate
from gis import getArea
...
class Surface(object):
    ...
    def garden(self, area, a=0.2, g=0.2, v=0.6, su='D'):
        '''
        ...
        validUse(su)
        b=get_b(su)
        surface_category=get_category(su)
    ...
    def roof(self, area, sp=0.3, su='D'):
        '''
    ...
    def flat_area(self, area, sp=1, su='D'):
        '''
    ...
    def green_roof(self, area, h, kf=70, wkmax_wp=0.5, su='D'):
        '''
    ...
    def storage_roof(self, area, sp=5, su='D'):
        '''
    ...
    def permeable_surface(self, area, fa, kf, sp=1, wkmax_wp=0.15, su='D'):
        '''
    ...
    def porous_surface(self, area, sp=3.5, h=100, kf=180, su='D'):
        '''
    ...
    def paver_stonegrid(self, area, fa=25, sp=1, wkmax_wp=0.15, su='D'):
        '''
    ...
    def gravel_cover(self, area, h=100, sp=3.5, kf=1.8, su='D'):
        '''
...
class Measure(object):      
    def __str__(self):
        return (
            ...
            "The following methods area available: drainage(), "
    ...
    def drainage(self, drainage_type, *surfaces, su='D'):
        '''
    ...
    def surf_infiltration(self, kf, *surfaces, fasf="fasf_standard", su='D'):
        '''
    ...
    # Infiltration swale
    def infilt_swale(self, kf, *surfaces, fasm="fasm_standard", su='D'):
        '''
    ...
    def swale_trench(self, kf, *surfaces, fasm="fasm_standard", su='D'):
        '''
    ...
    # Swale-trench system
    def swale_trench_system(self, qdr, kf, *surfaces, fasm="fasm_standard", su='D'):
        '''
        Calculates water balance components for swale-trench elements
    ...
    # Rainwater usage
    def rainwater_usage(self, vsp, vbr, *surfaces, fabw=2, qbw=60, su='D'):
        '''
...
def watbal(*study_areas):
        ...
        area, vp, va, vg, vv, ve, sc, Bafs63= 0, 0, 0, 0, 0, 0, 0, 0
        for df in study_areas:
            ...
            vv += float(sum(df[:]['Vv']))
            ve += float(sum(df[:]['Ve']))
            Bafs63 += float(sum(df[:]['Bafs63']))
            sc_max = float(max(df[:]['SC']))
        ...
        #Treatment of the runoff water leading into another body of water is only necessary for surface use category 2 or 3
        #

        if sc_max>=2:
            print('____________________')
            print('Treatment of the runoff water necessary due to occurance of surface category 2 or higher')
            print('____________________')
        else:
            print('____________________')
            print('No treatment of the runoff water necessary')
            print('____________________')

        sys_results = [{'Element' : 'System', 'Area' : round(area),
                    'a' : a, 'g' : g, 'v' : v, 'e' : e, 'Vp': round(vp),
                    'Va' : round(va),'Vg' : round(vg),'Vv' : round(vv),
                    'Ve' : round(ve),'SC': sc_max,'Bafs63':round(Bafs63)}]
        ...
        #Construction of Data Frames for specific Data about surface usage
        #as presented in the printed text

        cat1=pd.DataFrame(sys_results.loc[(sys_results['SC'] == 1)&(sys_results['Element'] != 'System')])
        cat1_sum_of_col=cat1.sum(axis=0)
        cat2 = pd.DataFrame(sys_results.loc[(sys_results['SC'] == 2) & (sys_results['Element'] != 'System')])
        cat2_sum_of_col = cat2.sum(axis=0)
        cat3 = pd.DataFrame(sys_results.loc[(sys_results['SC'] == 3) & (sys_results['Element'] != 'System')])
